...
// To install, run:
//
//     $ go install github.com/frogkind/godl/{{.Version}}@latest
//     $ {{.Version}} download
//
...
// of module github.com/frogkind/godl by invoking 'go list -m' in module mode.
// It must be called with a working directory that is contained
// by the github.com/frogkind/godl module, otherwise it returns an error.
func golangOrgDlRoot() (string, error) {
	...
	if mod.Path != "github.com/frogkind/godl" {
		return "", fmt.Errorf("working directory must be in module github.com/frogkind/godl, but 'go list -m' reports it's currently in module %s", mod.Path)
	}
	return mod.Dir, nil
}
